...
function _calcEffectBonus(card) {
  ...
  if (Array.isArray(card.effects)) {
    for (const eff of card.effects) {
      const dur = _toNum(eff.duration);
      switch (eff.type) {
        case 'burn': case 'poison': bonus += _toNum(eff.damagePerSecond) * dur * 1.2; break;
        case 'slow':                bonus += _toNum(eff.percentage) * dur * 1.5; break;
        case 'silence':             bonus += dur * 40; break;
        case 'blind':               bonus += dur * 30; break;
        case 'curse':               bonus += _toNum(eff.percentage) * dur * 2.0; break;
        case 'stun':                bonus += dur * 50; break;
        case 'heal':                bonus += _toNum(eff.amount) * 0.5; break;
        case 'stat-reduction':      bonus += _toNum(eff.amount) * 0.6; break;
        case 'shield-grant':        bonus += _toNum(eff.amount) * 0.4; break;
      }
    }
  }
