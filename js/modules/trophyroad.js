...
function initTrophyRoad() {
  ...
  setTimeout(() => {
    ...
    if (track) {
      track.scrollTop = track.scrollHeight; /* Arena 1 is at bottom */
      updateArenaOpacity();
      track.addEventListener('scroll', updateArenaOpacity, { passive: true });
    }
  }, 50);
}
...
function updateArenaOpacity() {
  ...
  groups.forEach(group => {
    ...
    const groupCenter = rect.top - trackRect.top + rect.height / 2;
    const distFromCenter = Math.abs(groupCenter - trackH / 2);
    const norm = Math.min(distFromCenter / (trackH * 0.65), 1);
