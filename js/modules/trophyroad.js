...
function initTrophyRoad() {
  ...
  setTimeout(() => {
    /* scroll so Arena 1 is visible first */
    const groups = document.querySelectorAll('.tr-arena-group');
    const last = groups[groups.length - 1];
    if (last) last.scrollIntoView({ behavior: 'instant', block: 'start' });
    updateArenaOpacity();
  }, 50);

  /* fade arenas based on scroll position */
  window.addEventListener('scroll', updateArenaOpacity, { passive: true });
}

function updateArenaOpacity() {
  const groups = document.querySelectorAll('.tr-arena-group');
  ...
  groups.forEach(group => {
    ...
    /* distance of group center from screen center (0 = perfectly centred) */
    const distFromCenter = Math.abs(groupCenter - vh / 2);
    /* normalise: 0 at centre, 1 when fully off screen */
    const norm = Math.min(distFromCenter / (vh * 0.72), 1);

    /* visible range: opacity 1→0.08 */
    const opacity = 1 - norm * 0.92;
    group.style.opacity = opacity;
    group.style.transform = `scale(${1 - norm * 0.025})`;
  });
}
...
function renderTR() {
  ...
  */
  ...
  [...ARENAS].reverse().forEach(arena => {
    ...
    /* wrap banner + steps in a single group for opacity control */
    const group = document.createElement('div');
    group.className = 'tr-arena-group';
    group.style.setProperty('--ac', arena.color);
