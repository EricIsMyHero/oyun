  ...
  if (Array.isArray(card.effects)) {
    for (const eff of card.effects) {
      ...
      switch (eff.type) {
        case "burn":
        case "poison":
          // Prioritet: "damage" (AxB) → "damagePerSecond" → "percentage"
          bonus += effDmg > 0 ? effDmg * 1.2 * cm
                 : dps   > 0 ? dps * dur * 1.2 * cm
                 :              pct * dur * 1.2 * cm;
          break;
        case "slow":
          bonus += pct * dur * 1.5 * cm;
          break;
        case "silence":
          // Həmişə duration * sabit çəki * chance — sadə və dəqiq
          bonus += dur * 40 * cm;
          break;
        case "blind":
          bonus += dur * 30 * cm;
